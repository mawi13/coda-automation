...
def metrics_to_values(metrics_order, metrics):
    ...
    values = [mapUsers(users) for users in metrics.keys()]
    values.insert(0, [''] + metrics_order)
    return values

def upload_new_metrics(sheet_name, combine_fns, new_metrics):
    sheet_id = '18VXeazbKykDuyu4RDabgq9y8Ox4lm_UnQqSWC8aMWaw'
    sheet_range = sheet_name + '!A1:Z'

    sheets = sheets_api.get_auth_sheets()
    values = sheets_api.get_sheet(sheets, sheet_id, sheet_range)
    ...
    sheets_api.set_sheet(sheets, sheet_id, sheet_range, new_values)
